...
//
// Attestation (docs/protection.md): a host that parked the module's own
// image bytes in guest RAM boots it with
//     argv = module.c4r attest <addr> <len> <sha256 hex>
// and main recomputes SHA-256 over those bytes before anything else. A
// mismatch is a refusal (status 3), and the host makes that loud. The hash
// below is the same function as runtime/sha256.js, in the c4 dialect (no
// unsigned: logical shifts are masked, bytes are masked from signed loads).
