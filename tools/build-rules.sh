...
if [ -n "$RELEASE_SEED" ]; then
  ...
  kt=$(mktemp -d)
  "$CF/tools/permute-release.sh" "$RELEASE_SEED" "$kt" "$here/vendor/coreframe/fw/fw.c4r" "$K/c4ke32.c4r" "$ktmp/mb_rules_k.c4r" $(for d in "${disk[@]}"; do echo "$K/disk/$d.c4r"; done) > /dev/null
